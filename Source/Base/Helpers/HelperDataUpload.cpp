...
HelperDataUpload::HelperDataUpload(Device& device, const StdAllocator<uint8_t>& stdAllocator, CommandQueue& commandQueue) :
...
Result HelperDataUpload::Create()
{
	...
	Result result = m_Device.CreateBuffer(bufferDesc, m_UploadBuffer);
	if (result != Result::SUCCESS)
		return result;

	MemoryDesc memoryDesc = {};
	(*m_UploadBuffer).GetMemoryInfo(MemoryLocation::HOST_UPLOAD, memoryDesc);

	result = m_Device.AllocateMemory(WHOLE_DEVICE_GROUP, memoryDesc.type, memoryDesc.size, m_UploadBufferMemory);
	if (result != Result::SUCCESS)
		return result;
	...
	result = m_Device.CreateDeviceSemaphore(false, m_DeviceSemaphore);
	if (result != Result::SUCCESS)
		return result;
	...
	{
		result = m_Device.CreateCommandAllocator(m_CommandQueue, WHOLE_DEVICE_GROUP, m_CommandAllocators[i]);
		if (result != Result::SUCCESS)
			return result;

		result = (*m_CommandAllocators[i]).CreateCommandBuffer(m_CommandBuffers[i]);
		if (result != Result::SUCCESS)
			return result;
	}
...
Result HelperDataUpload::UploadBuffers(const BufferUploadDesc* bufferUploadDescs, uint32_t bufferUploadDescNum)
{
	...
	while (i < bufferUploadDescNum)
	{
		...
		m_UploadBufferOffset = 0;
		m_MappedMemory = (uint8_t*)(*m_UploadBuffer).Map(0, m_UploadBufferSize);
...
Result HelperDataUpload::BeginCommandBuffers()
{
	...
	for (uint32_t i = 0; i < m_CommandBuffers.size() && result == Result::SUCCESS; i++)
		result = (*m_CommandBuffers[i]).Begin(nullptr, i);
...
Result HelperDataUpload::EndCommandBuffersAndSubmit()
{
	for (uint32_t i = 0; i < m_CommandBuffers.size(); i++)
	{
		const Result result = (*m_CommandBuffers[i]).End();
		if (result != Result::SUCCESS)
			return result;
	}
...
void HelperDataUpload::CopyTextureSubresourceContent(const TextureSubresourceUploadDesc& subresource, uint64_t alignedRowPitch, uint64_t alignedSlicePitch)
{
	...
	m_MappedMemory = (uint8_t*)(*m_UploadBuffer).Map(m_UploadBufferOffset, subresource.sliceNum * alignedSlicePitch);
...
Result HelperDataUpload::DoTransition(const TextureUploadDesc* textureUploadDescs, uint32_t textureDataDescNum)
{
	...
	for (uint32_t i = 0; i < textureDataDescNum;)
	{
		...
		for (uint32_t j = 0; j < m_CommandBuffers.size(); j++)
			(*m_CommandBuffers[j]).PipelineBarrier(&transitions, nullptr, BarrierDependency::COPY_STAGE);
	}
