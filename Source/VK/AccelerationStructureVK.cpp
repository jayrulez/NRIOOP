...
void AccelerationStructureVK::PrecreateBottomLevel(const AccelerationStructureDesc& accelerationStructureDesc)
{
    ...
    uint32_t* primitiveMaxNums = ALLOCATE_SCRATCH(m_Device.GetStdAllocator(), uint32_t, accelerationStructureDesc.instanceOrGeometryObjectNum);
    ...
    FREE_SCRATCH(m_Device.GetStdAllocator(), primitiveMaxNums, accelerationStructureDesc.instanceOrGeometryObjectNum);
}
