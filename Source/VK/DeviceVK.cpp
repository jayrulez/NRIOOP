...
DeviceVK::DeviceVK(const Logger& log, const StdAllocator<uint8_t>& stdAllocator) :
...
Result DeviceVK::CreateGraphicsPipeline(const GraphicsPipelineDesc& graphicsPipelineDesc, Pipeline*& pipeline)
{
    return CreateImplementation<PipelineVK>(pipeline, graphicsPipelineDesc);
}

Result DeviceVK::CreateComputePipeline(const ComputePipelineDesc& computePipelineDesc, Pipeline*& pipeline)
{
...
Result DeviceVK::CreateRayTracingPipeline(const RayTracingPipelineDesc& rayTracingPipelineDesc, Pipeline*& pipeline)
{
...
void DeviceVK::DestroyCommandBuffer(CommandBuffer& commandBuffer)
{
...
Result DeviceVK::BindAccelerationStructureMemory(const AccelerationStructureMemoryBindingDesc* memoryBindingDescs, uint32_t memoryBindingDescNum)
{
    ...
    BufferMemoryBindingDesc* infos = ALLOCATE_SCRATCH((*this).GetStdAllocator(), BufferMemoryBindingDesc, memoryBindingDescNum);
    ...
    FREE_SCRATCH((*this).GetStdAllocator(), infos, memoryBindingDescNum);
...
Result CreateDeviceVK(const DeviceCreationDesc& deviceCreationDesc, Device*& device)
{
    Logger log(GraphicsAPI::VULKAN, deviceCreationDesc.callbackInterface);
...
Result CreateDeviceVK(const DeviceCreationVulkanDesc& deviceCreationDesc, Device*& device)
{
    Logger log(GraphicsAPI::VULKAN, deviceCreationDesc.callbackInterface);
    StdAllocator<uint8_t> allocator(deviceCreationDesc.memoryAllocatorInterface);
