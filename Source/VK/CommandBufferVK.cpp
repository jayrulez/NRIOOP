...
{
    const VkAccelerationStructureKHR dstASHandle = (VkAccelerationStructureKHR)((const AccelerationStructureVK&)dst).GetHandle(m_PhysicalDeviceIndex);
    ...
    AccelerationStructureBuildBits flags, AccelerationStructure& dst, Buffer& scratch, uint64_t scratchOffset)
{
    ...
    VkAccelerationStructureGeometryKHR* geometries = ALLOCATE_SCRATCH(m_Device.GetStdAllocator(), VkAccelerationStructureGeometryKHR, geometryObjectNum);
    VkAccelerationStructureBuildRangeInfoKHR* ranges = ALLOCATE_SCRATCH(m_Device.GetStdAllocator(), VkAccelerationStructureBuildRangeInfoKHR, geometryObjectNum);
    ...
    FREE_SCRATCH(m_Device.GetStdAllocator(), ranges, geometryObjectNum);
    FREE_SCRATCH(m_Device.GetStdAllocator(), geometries, geometryObjectNum);
}
...
{
    const VkAccelerationStructureKHR srcASHandle = (VkAccelerationStructureKHR)((const AccelerationStructureVK&)src).GetHandle(m_PhysicalDeviceIndex);
    const VkAccelerationStructureKHR dstASHandle = (VkAccelerationStructureKHR)((const AccelerationStructureVK&)dst).GetHandle(m_PhysicalDeviceIndex);
    const VkDeviceAddress scratchAddress = ((BufferVK&)scratch).GetDeviceAddress(m_PhysicalDeviceIndex) + scratchOffset;
    const VkDeviceAddress bufferAddress = ((BufferVK&)buffer).GetDeviceAddress(m_PhysicalDeviceIndex) + bufferOffset;
...
{
    VkAccelerationStructureKHR* ASes = ALLOCATE_SCRATCH(m_Device.GetStdAllocator(), VkAccelerationStructureKHR, accelerationStructureNum);

    for (uint32_t i = 0; i < accelerationStructureNum; i++)
        ASes[i] = (VkAccelerationStructureKHR)((const AccelerationStructureVK*)accelerationStructures[i])->GetHandle(m_PhysicalDeviceIndex);
    ...
    FREE_SCRATCH(m_Device.GetStdAllocator(), ASes, accelerationStructureNum);
}
