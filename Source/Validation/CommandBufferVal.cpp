...
void CommandBufferVal::BeginRenderPass(const FrameBuffer& frameBuffer, RenderPassBeginFlag renderPassBeginFlag)
{
    ...
    m_ImplObject.BeginRenderPass(frameBuffer, renderPassBeginFlag);
}
    ...
    const TextureRegionDesc& srcRegionDesc)
{
    ...
    m_ImplObject.ReadbackTextureToBuffer(*dstBufferImpl, dstDataLayoutDesc, *srcTextureImpl, srcRegionDesc);
}
    ...
    AccelerationStructureBuildBits flags, AccelerationStructure& dst, Buffer& scratch, uint64_t scratchOffset)
{
    ...
    m_ImplObject.BuildBottomLevelAccelerationStructure(geometryObjectNum, objectImplArray.data(), flags, dstImpl, scratchImpl, scratchOffset);
}
    ...
    AccelerationStructureBuildBits flags, AccelerationStructure& dst, AccelerationStructure& src, Buffer& scratch, uint64_t scratchOffset)
{
    ...
    m_ImplObject.UpdateBottomLevelAccelerationStructure(geometryObjectNum, objectImplArray.data(), flags, dstImpl, srcImpl, scratchImpl, scratchOffset);
}
...
void CommandBufferVal::DispatchMeshTasks(uint32_t taskNum)
{
    ...
    m_ImplObject.DispatchMeshTasks(std::min(taskNum, meshTaskMaxNum));
}
