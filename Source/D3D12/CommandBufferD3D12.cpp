...
extern DXGI_FORMAT GetDXGIFormat(Format format);
...
Result CommandBufferD3D12::Begin(const DescriptorPool* descriptorPool, uint32_t physicalDeviceIndex)
{
...
void CommandBufferD3D12::SetViewports(const Viewport* viewports, uint32_t viewportNum)
{
...
void CommandBufferD3D12::SetScissors(const Rect* rects, uint32_t rectNum)
{
...
void CommandBufferD3D12::SetDepthBounds(float boundsMin, float boundsMax)
{
    if (m_GraphicsCommandList1)
        m_GraphicsCommandList1->OMSetDepthBounds(boundsMin, boundsMax);
}

void CommandBufferD3D12::SetStencilReference(uint8_t reference)
{
    m_GraphicsCommandList->OMSetStencilRef(reference);
}

void CommandBufferD3D12::SetSamplePositions(const SamplePosition* positions, uint32_t positionNum)
{
    if (m_GraphicsCommandList1)
    {
...
void CommandBufferD3D12::ClearAttachments(const ClearDesc* clearDescs, uint32_t clearDescNum, const Rect* rects, uint32_t rectNum)
{
    m_FrameBuffer->Clear(m_GraphicsCommandList, clearDescs, clearDescNum, rects, rectNum);
}

void CommandBufferD3D12::ClearStorageBuffer(const ClearStorageBufferDesc& clearDesc)
{
...
void CommandBufferD3D12::ClearStorageTexture(const ClearStorageTextureDesc& clearDesc)
{
    DescriptorSetD3D12* descriptorSet = m_DescriptorSets[clearDesc.setIndex];
...
void CommandBufferD3D12::BeginRenderPass(const FrameBuffer& frameBuffer, RenderPassBeginFlag renderPassBeginFlag)
{
...
void CommandBufferD3D12::SetVertexBuffers(uint32_t baseSlot, uint32_t bufferNum, const Buffer* const* buffers, const uint64_t* offsets)
{
...
void CommandBufferD3D12::SetIndexBuffer(const Buffer& buffer, uint64_t offset, IndexType indexType)
{
...
void CommandBufferD3D12::SetPipelineLayout(const PipelineLayout& pipelineLayout)
{
...
void CommandBufferD3D12::SetPipeline(const Pipeline& pipeline)
{
...
void CommandBufferD3D12::SetDescriptorPool(const DescriptorPool& descriptorPool)
{
    ((DescriptorPoolD3D12&)descriptorPool).Bind(m_GraphicsCommandList);
}

void CommandBufferD3D12::SetDescriptorSets(uint32_t baseIndex, uint32_t setNum, const DescriptorSet* const* descriptorSets, const uint32_t* offsets)
{
...
void CommandBufferD3D12::SetConstants(uint32_t pushConstantRangeIndex, const void* data, uint32_t size)
{
...
void CommandBufferD3D12::Draw(uint32_t vertexNum, uint32_t instanceNum, uint32_t baseVertex, uint32_t baseInstance)
{
    m_GraphicsCommandList->DrawInstanced(vertexNum, instanceNum, baseVertex, baseInstance);
}

void CommandBufferD3D12::DrawIndexed(uint32_t indexNum, uint32_t instanceNum, uint32_t baseIndex, uint32_t baseVertex, uint32_t baseInstance)
{
    m_GraphicsCommandList->DrawIndexedInstanced(indexNum, instanceNum, baseIndex, baseVertex, baseInstance);
}

void CommandBufferD3D12::DrawIndirect(const Buffer& buffer, uint64_t offset, uint32_t drawNum, uint32_t stride)
{
...
void CommandBufferD3D12::DrawIndexedIndirect(const Buffer& buffer, uint64_t offset, uint32_t drawNum, uint32_t stride)
{
...
void nri::CommandBufferD3D12::CopyBuffer(Buffer& dstBuffer, uint32_t dstPhysicalDeviceIndex, uint64_t dstOffset, const Buffer& srcBuffer, uint32_t srcPhysicalDeviceIndex, uint64_t srcOffset, uint64_t size)
{
    if (size == WHOLE_SIZE)
...
void nri::CommandBufferD3D12::CopyTexture(Texture& dstTexture, uint32_t dstPhysicalDeviceIndex, const TextureRegionDesc* dstRegionDesc, const Texture& srcTexture, uint32_t srcPhysicalDeviceIndex, const TextureRegionDesc* srcRegionDesc)
{
    TextureD3D12& dstTextureD3D12 = (TextureD3D12&)dstTexture;
    TextureD3D12& srcTextureD3D12 = (TextureD3D12&)srcTexture;

    if (!dstRegionDesc || !srcRegionDesc)
    {
    ...
    else
    {
        ...
        const uint16_t(&dstOffset)[3] = dstRegionDesc->offset;
...
void CommandBufferD3D12::UploadBufferToTexture(Texture& dstTexture, const TextureRegionDesc& dstRegionDesc, const Buffer& srcBuffer, const TextureDataLayoutDesc& srcDataLayoutDesc)
{
    TextureD3D12& dstTextureD3D12 = (TextureD3D12&)dstTexture;
...
void CommandBufferD3D12::ReadbackTextureToBuffer(Buffer& dstBuffer, TextureDataLayoutDesc& dstDataLayoutDesc, const Texture& srcTexture, const TextureRegionDesc& srcRegionDesc)
{
    TextureD3D12& srcTextureD3D12 = (TextureD3D12&)srcTexture;
...
void CommandBufferD3D12::Dispatch(uint32_t x, uint32_t y, uint32_t z)
{
    m_GraphicsCommandList->Dispatch(x, y, z);
}

void CommandBufferD3D12::DispatchIndirect(const Buffer& buffer, uint64_t offset)
{
    m_GraphicsCommandList->ExecuteIndirect(m_Device.GetDispatchCommandSignature(), 1, (BufferD3D12&)buffer, offset, nullptr, 0);
}

void CommandBufferD3D12::PipelineBarrier(const TransitionBarrierDesc* transitionBarriers, const AliasingBarrierDesc* aliasingBarriers, BarrierDependency dependency)
{
...
void CommandBufferD3D12::BeginQuery(const QueryPool& queryPool, uint32_t offset)
{
...
void CommandBufferD3D12::EndQuery(const QueryPool& queryPool, uint32_t offset)
{
    const QueryPoolD3D12& queryPoolD3D12 = (QueryPoolD3D12&)queryPool;
    m_GraphicsCommandList->EndQuery(queryPoolD3D12, queryPoolD3D12.GetType(), offset);
}

void CommandBufferD3D12::CopyQueries(const QueryPool& queryPool, uint32_t offset, uint32_t num, Buffer& buffer, uint64_t alignedBufferOffset)
{
    const QueryPoolD3D12& queryPoolD3D12 = (QueryPoolD3D12&)queryPool;
    const BufferD3D12& bufferD3D12 = (BufferD3D12&)buffer;
...
void CommandBufferD3D12::BeginAnnotation(const char* name)
{
...
void CommandBufferD3D12::EndAnnotation()
{
...
void CommandBufferD3D12::WriteAccelerationStructureSize(const AccelerationStructure* const* accelerationStructures, uint32_t accelerationStructureNum, QueryPool& queryPool, uint32_t queryOffset)
{
...
#ifdef __ID3D12GraphicsCommandList4_INTERFACE_DEFINED__
    D3D12_GPU_VIRTUAL_ADDRESS* virtualAddresses = ALLOCATE_SCRATCH(m_Device.GetStdAllocator(), D3D12_GPU_VIRTUAL_ADDRESS, accelerationStructureNum);
    ...
    FREE_SCRATCH(m_Device.GetStdAllocator(), virtualAddresses, accelerationStructureNum);
#endif
}
...
void CommandBufferD3D12::DispatchMeshTasks(uint32_t taskNum)
{
#ifdef __ID3D12GraphicsCommandList6_INTERFACE_DEFINED__
...
void CommandBufferD3D12::AddResourceBarrier(ID3D12Resource* resource, AccessBits before, AccessBits after, D3D12_RESOURCE_BARRIER& resourceBarrier, uint32_t subresource)
{
