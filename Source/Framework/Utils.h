...
namespace utils
{
    ...
    NRI_FRAMEWORK_API const char* GetFileName(const std::string& path);
    NRI_FRAMEWORK_API std::string GetFullPath(const std::string& localPath, DataFolder dataFolder);
    NRI_FRAMEWORK_API bool LoadFile(const std::string& path, std::vector<uint8_t>& data);
    NRI_FRAMEWORK_API nri::ShaderDesc LoadShader(nri::GraphicsAPI graphicsAPI, const std::string& path, ShaderCodeStorage& storage, const char* entryPointName = nullptr);
    NRI_FRAMEWORK_API bool LoadTexture(const std::string& path, Texture& texture, bool computeAvgColorAndAlphaMode = false);
    NRI_FRAMEWORK_API void LoadTextureFromMemory(nri::Format format, uint32_t width, uint32_t height, const uint8_t* pixels, Texture& texture);
    NRI_FRAMEWORK_API bool LoadScene(const std::string& path, Scene& scene, bool simpleOIT = false, const std::vector<float3>& instanceData = { float3(0.0f, 0.0f, 0.0f), float3(0.0f, 0.0f, 0.0f) });
