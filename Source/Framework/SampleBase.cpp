...
bool SampleBase::CreateUserInterface(nri::Device& device, uint32_t windowWidth, uint32_t windowHeight, nri::Format renderTargetFormat)
{
    // ImGui setup
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ...
    const nri::DeviceDesc& deviceDesc = device.GetDesc();
    ...
    // Pipeline
    {
        ...
        if (device.CreatePipelineLayout(pipelineLayoutDesc, m_PipelineLayout) != nri::Result::SUCCESS)
            return false;
        ...
        if (device.CreateGraphicsPipeline(graphicsPipelineDesc, m_Pipeline) != nri::Result::SUCCESS)
            return false;
    }
    ...
    constexpr nri::Format format = nri::Format::RGBA8_UNORM;
    {
        ...
        if (device.CreateBuffer(bufferDesc, m_GeometryBuffer) != nri::Result::SUCCESS)
            return false;
        ...
        if (device.CreateTexture(textureDesc, m_FontTexture) != nri::Result::SUCCESS)
            return false;
    }
    ...
    nri::Result result = device.AllocateAndBindMemory(resourceGroupDesc, m_MemoryAllocations.data());
    ...
    result = device.AllocateAndBindMemory(resourceGroupDesc, m_MemoryAllocations.data() + 1);
    if (result != nri::Result::SUCCESS)
        return false;
    ...
    // Descriptor
    {
        ...
        if (device.CreateTexture2DView(texture2DViewDesc, m_FontShaderResource) != nri::Result::SUCCESS)
            return false;
    }
    ...
    device.GetCommandQueue(nri::CommandQueueType::GRAPHICS, commandQueue);
    ...
    // Upload data
    {
        ...
        if ((*commandQueue).UploadData(&textureData, 1, nullptr, 0) != nri::Result::SUCCESS)
            return false;
    }
    ...
    // Descriptor pool
    {
        ...
        if (device.CreateDescriptorPool(descriptorPoolDesc, m_DescriptorPool) != nri::Result::SUCCESS)
            return false;
    }

    // Texture & sampler descriptor set
    {
        if ((*m_DescriptorPool).AllocateDescriptorSets(*m_PipelineLayout, 0, &m_DescriptorSet, 1, nri::WHOLE_DEVICE_GROUP, 0) != nri::Result::SUCCESS)
            return false;
        ...
        (*m_DescriptorSet).UpdateDescriptorRanges(nri::WHOLE_DEVICE_GROUP, 0, 1, &descriptorRangeUpdateDesc);
    }
...
void SampleBase::DestroyUserInterface()
{
    ...
    if (m_FontShaderResource)
        m_Device->DestroyDescriptor(*m_FontShaderResource);
    ...
    for (uint32_t i = 0; i < m_MemoryAllocations.size(); i++)
        m_Device->FreeMemory(*m_MemoryAllocations[i]);
}
...
void SampleBase::RenderUserInterface(nri::CommandBuffer& commandBuffer)
{
    ...
    uint8_t* indexData = (uint8_t*)(*m_GeometryBuffer).Map(m_StreamBufferOffset, totalDataSizeAligned);
    ...
    {
        helper::Annotation(commandBuffer, "UserInterface");

        commandBuffer.SetDescriptorPool(*m_DescriptorPool);
        commandBuffer.SetPipelineLayout( *m_PipelineLayout);
        commandBuffer.SetPipeline( *m_Pipeline);
        commandBuffer.SetConstants( 0, invScreenSize, sizeof(invScreenSize));
        commandBuffer.SetIndexBuffer( *m_GeometryBuffer, indexBufferOffset, sizeof(ImDrawIdx) == 2 ? nri::IndexType::UINT16 : nri::IndexType::UINT32);
        commandBuffer.SetVertexBuffers( 0, 1, &m_GeometryBuffer, &vertexBufferOffset);
        commandBuffer.SetDescriptorSets( 0, 1, &m_DescriptorSet, nullptr);
        ...
        for (int32_t n = 0; n < drawData.CmdListsCount; n++)
        {
            ...
            for (int32_t i = 0; i < drawList.CmdBuffer.Size; i++)
            {
                ...
                else
                {
                    ...
                    commandBuffer.DrawIndexed( drawCmd.ElemCount, 1, indexOffset, vertexOffset, 0);
                }
