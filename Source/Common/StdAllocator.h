...
#define ALLOCATE_SCRATCH(allocator, T, arraySize) \
    ...
    AllocateArray<T>(allocator, arraySize);

#define FREE_SCRATCH(allocator, array, arraySize) \
    if (array != nullptr && CountStackAllocationSize<decltype(array[0])>(arraySize) > STACK_ALLOC_MAX_SIZE) \
        DeallocateArray(allocator, array, arraySize);
