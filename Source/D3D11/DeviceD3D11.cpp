...
DeviceD3D11::DeviceD3D11(const Logger& log, StdAllocator<uint8_t>& stdAllocator) :
...
inline Result DeviceD3D11::CreateCommandAllocator(const CommandQueue& commandQueue, uint32_t physicalDeviceMask, CommandAllocator*& commandAllocator)
{
...
inline Result DeviceD3D11::CreateBufferView(const BufferViewDesc& bufferViewDesc, Descriptor*& bufferView)
{
    return CreateImplementationWithNonEmptyConstructor<DescriptorD3D11>(bufferView, *this, m_Device, bufferViewDesc);
}

inline Result DeviceD3D11::CreateTexture1DView(const Texture1DViewDesc& textureViewDesc, Descriptor*& textureView)
{
    return CreateImplementationWithNonEmptyConstructor<DescriptorD3D11>(textureView, *this, m_Device, textureViewDesc);
}

inline Result DeviceD3D11::CreateTexture2DView(const Texture2DViewDesc& textureViewDesc, Descriptor*& textureView)
{
    return CreateImplementationWithNonEmptyConstructor<DescriptorD3D11>(textureView, *this, m_Device, textureViewDesc);
}

inline Result DeviceD3D11::CreateTexture3DView(const Texture3DViewDesc& textureViewDesc, Descriptor*& textureView)
{
    return CreateImplementationWithNonEmptyConstructor<DescriptorD3D11>(textureView, *this, m_Device, textureViewDesc);
}

inline Result DeviceD3D11::CreateSampler(const SamplerDesc& samplerDesc, Descriptor*& sampler)
{
...
inline Result DeviceD3D11::CreateGraphicsPipeline(const GraphicsPipelineDesc& graphicsPipelineDesc, Pipeline*& pipeline)
{
...
inline Result DeviceD3D11::CreateComputePipeline(const ComputePipelineDesc& computePipelineDesc, Pipeline*& pipeline)
{
    PipelineD3D11* implementation = Allocate<PipelineD3D11>(GetStdAllocator(), *this, &m_Device);
...
void DeviceD3D11::DestroyCommandBuffer(CommandBuffer& commandBuffer)
{
...
inline Result DeviceD3D11::AllocateMemory(uint32_t physicalDeviceMask, MemoryType memoryType, uint64_t size, Memory*& memory)
{
    MaybeUnused(size);
...
Result CreateDeviceD3D11(const DeviceCreationDesc& deviceCreationDesc, Device*& device)
{
    Logger log(GraphicsAPI::D3D11, deviceCreationDesc.callbackInterface);
...
Result CreateDeviceD3D11(const DeviceCreationD3D11Desc& deviceCreationD3D11Desc, Device*& device)
{
