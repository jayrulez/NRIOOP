...
Sample::~Sample()
{
    ...
    {
        (*m_Device).DestroyCommandBuffer(*frame.commandBuffer);
        (*m_Device).DestroyCommandAllocator(*frame.commandAllocator);
        (*m_Device).DestroyDeviceSemaphore(*frame.deviceSemaphore);
        (*m_Device).DestroyDescriptor(*frame.constantBufferView);
    }

    for (BackBuffer& backBuffer : m_SwapChainBuffers)
    {
        (*m_Device).DestroyFrameBuffer(*backBuffer.frameBuffer);
        (*m_Device).DestroyDescriptor(*backBuffer.colorAttachment);
    }

    (*m_Device).DestroyPipeline(*m_Pipeline);
    (*m_Device).DestroyPipelineLayout(*m_PipelineLayout);
    (*m_Device).DestroyDescriptor(*m_TextureShaderResource);
    (*m_Device).DestroyDescriptor(*m_Sampler);
    (*m_Device).DestroyBuffer(*m_ConstantBuffer);
    (*m_Device).DestroyBuffer(*m_GeometryBuffer);
    (*m_Device).DestroyTexture(*m_Texture);
    (*m_Device).DestroyDescriptorPool(*m_DescriptorPool);
    (*m_Device).DestroyQueueSemaphore(*m_AcquireSemaphore);
    (*m_Device).DestroyQueueSemaphore(*m_ReleaseSemaphore);
...
bool Sample::Initialize(nri::GraphicsAPI graphicsAPI)
{
    ...
    // Command queue
    NRI_ABORT_ON_FAILURE((*m_Device).GetCommandQueue(nri::CommandQueueType::GRAPHICS, m_CommandQueue) );
    ...
    nri::Format swapChainFormat;
    {
        ...
        NRI_ABORT_ON_FAILURE( (*m_Device).CreateSwapChain(swapChainDesc, m_SwapChain) );

        uint32_t swapChainTextureNum;
        nri::Texture* const* swapChainTextures = (*m_SwapChain).GetTextures(swapChainTextureNum, swapChainFormat);
        ...
        for (uint32_t i = 0; i < swapChainTextureNum; i++)
        {
            ...
            NRI_ABORT_ON_FAILURE( (*m_Device).CreateTexture2DView(textureViewDesc, colorAttachment) );
            ...
            NRI_ABORT_ON_FAILURE( (*m_Device).CreateFrameBuffer(frameBufferDesc, frameBuffer) );
    ...
    NRI_ABORT_ON_FAILURE( (*m_Device).CreateQueueSemaphore(m_AcquireSemaphore) );
    NRI_ABORT_ON_FAILURE( (*m_Device).CreateQueueSemaphore(m_ReleaseSemaphore) );

    // Buffered resources
    for (Frame& frame : m_Frames)
    {
        NRI_ABORT_ON_FAILURE((*m_Device).CreateDeviceSemaphore(true, frame.deviceSemaphore) );
        NRI_ABORT_ON_FAILURE((*m_Device).CreateCommandAllocator(*m_CommandQueue, nri::WHOLE_DEVICE_GROUP, frame.commandAllocator) );
        NRI_ABORT_ON_FAILURE( (*frame.commandAllocator).CreateCommandBuffer(frame.commandBuffer) );
    }

    // Pipeline
    const nri::DeviceDesc& deviceDesc = (*m_Device).GetDesc();
    utils::ShaderCodeStorage shaderCodeStorage;
    {
        ...
        NRI_ABORT_ON_FAILURE((*m_Device).CreatePipelineLayout(pipelineLayoutDesc, m_PipelineLayout));
        ...
        NRI_ABORT_ON_FAILURE( (*m_Device).CreateGraphicsPipeline(graphicsPipelineDesc, m_Pipeline) );
    }
    ...
    // Descriptor pool
    {
        ...
        NRI_ABORT_ON_FAILURE( (*m_Device).CreateDescriptorPool(descriptorPoolDesc, m_DescriptorPool) );
    }
    ...
    const uint64_t vertexDataSize = sizeof(g_VertexData);
    {
        ...
        NRI_ABORT_ON_FAILURE( (*m_Device).CreateTexture(textureDesc, m_Texture) );
        ...
        // Constant buffer
        {
            ...
            NRI_ABORT_ON_FAILURE( (*m_Device).CreateBuffer(bufferDesc, m_ConstantBuffer) );
        }
        ...
        // Geometry buffer
        {
            ...
            NRI_ABORT_ON_FAILURE((*m_Device).CreateBuffer(bufferDesc, m_GeometryBuffer) );
        }
    ...
    NRI_ABORT_ON_FAILURE( (*m_Device).AllocateAndBindMemory(resourceGroupDesc, m_MemoryAllocations.data()) );
    ...
    m_MemoryAllocations.resize(1 + (*m_Device).CalculateAllocationNumber(resourceGroupDesc), nullptr);
    NRI_ABORT_ON_FAILURE( (*m_Device).AllocateAndBindMemory(resourceGroupDesc, m_MemoryAllocations.data() + 1) );

    // Descriptors
    {
        // Texture
        nri::Texture2DViewDesc texture2DViewDesc = {m_Texture, nri::Texture2DViewType::SHADER_RESOURCE_2D, texture.GetFormat()};
        NRI_ABORT_ON_FAILURE( (*m_Device).CreateTexture2DView(texture2DViewDesc, m_TextureShaderResource) );
        ...
        NRI_ABORT_ON_FAILURE( (*m_Device).CreateSampler(samplerDesc, m_Sampler) );
        ...
        for (uint32_t i = 0; i < BUFFERED_FRAME_MAX_NUM; i++)
        {
            ...
            NRI_ABORT_ON_FAILURE( (*m_Device).CreateBufferView(bufferViewDesc, m_Frames[i].constantBufferView));
    ...
    {
        // Texture
        NRI_ABORT_ON_FAILURE( (*m_DescriptorPool).AllocateDescriptorSets(*m_PipelineLayout, 1,
        ...
        (*m_TextureDescriptorSet).UpdateDescriptorRanges(nri::WHOLE_DEVICE_GROUP, 0, helper::GetCountOf(descriptorRangeUpdateDescs), descriptorRangeUpdateDescs);

        // Constant buffer
        for (Frame& frame : m_Frames)
        {
            NRI_ABORT_ON_FAILURE( (*m_DescriptorPool).AllocateDescriptorSets(*m_PipelineLayout, 0, &frame.constantBufferDescriptorSet, 1, nri::WHOLE_DEVICE_GROUP, 0) );

            nri::DescriptorRangeUpdateDesc descriptorRangeUpdateDesc = { &frame.constantBufferView, 1 };
            (*frame.constantBufferDescriptorSet).UpdateDescriptorRanges(nri::WHOLE_DEVICE_GROUP, 0, 1, &descriptorRangeUpdateDesc);
        }
    }
    ...
    // Upload data
    {
        ...
        NRI_ABORT_ON_FAILURE((*m_CommandQueue).UploadData(&textureData, 1, &bufferData, 1));
    }

     //User interface
    bool initialized = CreateUserInterface(*m_Device, GetWindowWidth(), GetWindowHeight(), swapChainFormat);
...
void Sample::RenderFrame(uint32_t frameIndex)
{
    ...
    const uint32_t currentTextureIndex = (*m_SwapChain).AcquireNextTexture(*m_AcquireSemaphore);
    ...
    (*commandBuffer).Begin(m_DescriptorPool, 0);
    {
        ...
        (*commandBuffer).PipelineBarrier(&transitionBarriers, nullptr, nri::BarrierDependency::ALL_STAGES);

        (*commandBuffer).BeginRenderPass(*currentBackBuffer.frameBuffer, nri::RenderPassBeginFlag::NONE);
        {
            {
                helper::Annotation annotation(*commandBuffer, "Clear");
                ...
                (*commandBuffer).ClearAttachments(&clearDesc, 1, rects, helper::GetCountOf(rects));
            }

            {
                helper::Annotation annotation(*commandBuffer, "Triangle");
                ...
                (*commandBuffer).SetIndexBuffer(*m_GeometryBuffer, 0, nri::IndexType::UINT16);
                ...
                (*commandBuffer).SetDescriptorSets( 0, helper::GetCountOf(sets), sets, nullptr);
